...
class PatronStatusResponse extends ResponseMessage {
  ...
  parse(message) {
    ...
    if (this.existsAndNotEmpty('BL', this.message.substring(37))) {
      const temp = this.parseVariable('BL', this.message.substring(37));
      data.validPatron = this.charToBool(temp.charAt(0));
      data.validPatronUsed = true;
    }
    if (this.existsAndNotEmpty('CQ', this.message.substring(37))) {
      const temp = this.parseVariable('CQ', this.message.substring(37));
